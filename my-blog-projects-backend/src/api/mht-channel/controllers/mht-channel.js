...
module.exports = createCoreController('api::mht-channel.mht-channel', ({ strapi }) => ({
  async find(ctx){
    const { query } = ctx;
    ...
    if(query.orderBy == 'initialMeetingDate-asc')
    {
      filters.orderBy = [{ initial_meeting_date: 'asc' }];
    }
    if(query.orderBy == 'initialMeetingDate-desc')
    {
      filters.orderBy = [{ initial_meeting_date: 'desc' }];
    }
    ...
    if(query.orderBy != 'name-desc' && query.orderBy != 'initialMeetingDate-asc' && query.orderBy != 'initialMeetingDate-desc' && query.orderBy != 'createdAtDate-asc' && query.orderBy != 'createdAtDate-desc')
    {
      filters.orderBy = [{ name: 'asc' }];
    }

    const entity = await strapi.db.query('api::mht-channel.mht-channel').findMany(filters);
  ...
  async findBySlug(ctx){
    ...
    const entity = await strapi.db.query('api::mht-channel.mht-channel').findOne({
      select: ['name', 'description', 'initial_meeting_date', 'initial_meeting_date_format', 'review', 'description', 'slug', 'locale', 'url', 'createdAt'],
      where: {
        slug,
        locale,
