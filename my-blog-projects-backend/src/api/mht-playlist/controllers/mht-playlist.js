...
module.exports = createCoreController('api::mht-playlist.mht-playlist', ({ strapi }) => ({
  async find(ctx){
    const { query } = ctx;
    ...
    var filters = {
      select: ['name', 'description', 'review', 'description', 'slug', 'grade', 'locale', 'createdAt'],
      where: {
        locale,
        publishedAt: {
          $notNull: true
    ...
    const entity = await strapi.db.query('api::mht-playlist.mht-playlist').findMany(filters);
  ...
  async findBySlug(ctx){
    ...
    const entity = await strapi.db.query('api::mht-playlist.mht-playlist').findOne({
      select: ['name', 'description', 'review', 'description', 'slug', 'grade', 'locale', 'createdAt'],
      where: {
        slug,
        locale,
